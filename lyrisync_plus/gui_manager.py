...
class LyriSyncGUI:
    ...
    def refresh_preach_list(self):
        try:
            ...
            for row in self.preach_db.list_entries():
    ...
    def _selected_preach_row(self) -> Optional[Dict[str, Any]]:
        ...
        if not sel:
            return None
        values = self.preach_tree.item(sel[0], "values") or ()
        if not values:
            return None
        try:
            row_id = int(values[0])
            ...
            return None
        return self.preach_db.get_entry(row_id)
    ...
    def _apply_settings(self, new_settings: Dict[str, Any]):
        old_db_path = (self.config.get("settings") or {}).get("preach_db_path", "lyrisync_preach.db")
        self.config["settings"] = new_settings
        new_db_path = (new_settings or {}).get("preach_db_path", "lyrisync_preach.db")
        if old_db_path != new_db_path:
            self.preach_db = PreachInfoDB(new_db_path)
            self.refresh_preach_list()
        save_config(self.config)
...
class SettingsDialog:
    def __init__(self, parent, config: Dict[str, Any], discoverer: AsyncVmixDiscoverer, on_apply: Callable[[Dict[str, Any]], None]):
        ...
        self.preach_db_path_var = tk.StringVar(value=s.get("preach_db_path", "lyrisync_preach.db"))
    ...
    def show(self):
        ...
        ttk.Label(main, text="Preach DB Path:").grid(row=19, column=0, sticky="w", pady=5)
        ttk.Entry(main, textvariable=self.preach_db_path_var, width=44).grid(row=19, column=1, sticky="ew", padx=6)

        # JSON / Quick Add
        sep = ttk.Separator(main); sep.grid(row=20, column=0, columnspan=4, sticky="ew", pady=(14, 8))
        ttk.Label(main, text="Multi-Connection", font=("Segoe UI", 12, "bold")).grid(row=21, column=0, columnspan=4, sticky="w", pady=(0, 8))
        self.conn_info_var = tk.StringVar(value=self._connections_summary())
        ttk.Label(main, textvariable=self.conn_info_var).grid(row=22, column=0, columnspan=2, sticky="w")
        btn_group = ttk.Frame(main); btn_group.grid(row=22, column=2, columnspan=2, sticky="e")
        ...
        # Bottom buttons
        btns = ttk.Frame(main)
        btns.grid(row=23, column=0, columnspan=4, pady=18, sticky="e")
